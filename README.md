...
- **정의**: 
  - 대용량 데이터 처리를 위한 오픈 소스 프레임워크로, 분산 저장과 처리 시스템을 기반으로 함. 데이터 저장을 위해 HDFS(Hadoop Distributed File System)를 사용하며, MapReduce 프로그래밍 모델을 통해 데이터를 분석함.

### 주요 구성 요소

1. **HDFS**: 
   - 대용량 데이터의 분산 저장을 위한 파일 시스템으로, 데이터의 신뢰성과 내구성을 보장함.

2. **MapReduce**: 
   - 대규모 데이터셋을 처리하기 위한 프로그래밍 모델로, 데이터를 분산 처리하여 효율성을 극대화함.

3. **YARN**: 
   - 자원 관리 시스템으로, 클러스터의 자원을 효율적으로 할당하고 관리함.

### 장점

- **확장성**: 
  - 노드를 추가함으로써 용량과 처리 능력을 손쉽게 확장할 수 있음.

- **비용 효율성**: 
  - 일반 하드웨어에서 운영할 수 있어 데이터 처리 비용을 절감할 수 있음.

- **유연성**: 
  - 구조화된 데이터뿐만 아니라 비정형 데이터도 저장하고 처리할 수 있음.
...
| 이름 | 역할 |   
| --- | --- |
| 나경준 | [팀장] 소셜 로그인, 회원 관리, 1원 인증, QR코드, S3, 발표 | 
| 곽재은 | 주식 기능, 알림, 공통 컴포넌트, PWA | 
| 김신범 | 주식 기능, 알림, Spring Boot 셋팅 | 
| 정지영 | 예적금, 대출 기능, Spring Boot, DB, UCC 제작 |
| 황우성 | 예적금, 대출 기능, Spring Boot, DB, 하둡, 스파크, SQOOP, 금융 퀴즈 백로직 |  
| 정소영 | 인프라 CI/CD 구축, 회원 & 전체 페이지 프런트 디자인, 이벤트 (퀴즈,게임,룰렛) |  
...
| 황우성 | 금융 퀴즈 기능의 백엔드 로직을 개발하면서, 알고리즘의 최적화와 데이터 처리 방식에 대해 많은 고민을 했습니다. 특히 자산 및 금융 기능을 구현하는 과정에서 금융 데이터의 정밀한 처리와 보안의 중요성을 깨달았습니다. 이번 프로젝트에서 Spring Boot와 데이터베이스 셋팅을 처음부터 진행해보면서 서버 환경 구성이 얼마나 중요한지 깊이 있게 이해할 수 있었습니다. 더불어 팀원들과의 협력 속에서 상호 존중하는 분위기가 프로젝트의 성공을 이끌어낸 것 같습니다. 앞으로도 이러한 팀워크를 잊지 않고 유지하고 싶습니다. | 
