...
| 이름 | 역할 |   
| --- | --- |
| 나경준 | [팀장]<br/> 카카오 소셜 로그인 기능 <br/>회원 관리 로직 제작 <br/> QR코드를 활용한 가족 연결 기능<br/>간편 비밀번호 제작 <br/> 입금계좌 등록 및 1원 인증 기능 구현<br/> S3를 활용한 데이터 관리 <br/> 전문가 리뷰 사전질문 및 멘토-멘티 활동일지 작성 <br/>  PPT 제작 및 프로젝트 발표 | 
| 곽재은 | [프런트 리더] <br/>주식 분석 및 종목 확인 기능 (차트 포함) <br/> 주식 매수 및 매도 기능 (거래내역 및 주문현황) <br/>알림 기능 (예적금 가입, 대출 심사, 출금 요청 등)<br/> Vue.js 공통 컴포넌트 제작, PWA 설정 <br/> 팀 내 컨벤션 관리 및 Code Formatting 담당 <br/> UCC 서포트| 
| 김신범 | 주식 분석 및 종목 확인 기능 (차트 포함)  <br/> 주식 매수 및 매도 기능 (거래내역 및 주문현황) <br/>알림 기능 (예적금 가입, 대출 심사, 출금 요청 등)<br/>  Chat GPT를 활용한 뉴스 요약 기능 <br/> Jira 관리 및 작성 독려| 
| 정지영 | 입금 계좌 관리 기능  <br/>예금, 적금 가입 및 해지  <br/> 예적금 상품 조회 기능  <br/>  대출 신청 및 승인 기능<br/>원리금균등상환, 만기일시상환 기능 구현 <br/> 자산 분석 및 자산추이 그래프 구현 <br/>UCC 제작, ERD 회의 기록 |
| 황우성 | 입금 계좌 관리 기능  <br/>예금, 적금 가입 및 해지 <br/>예적금 상품 조회 기능  <br/> 대출 신청 및 승인 기능<br/>원리금균등상환, 만기일시상환 기능 구현<br/> 자산 분석 및 자산추이 그래프 구현 <br/>사용자 금융 데이터 분산처리 및 유형 분류 (초보자, 안전 제일, 꾸준한 저축가, 미래의 투자가)<br/> 하둡, 스파크, SQOOP를 활용한 예적금 추천 알고리즘 제작<br/>경제 용어 퀴즈 백로직 및 DB 구현<br/>인프라 서포트 |  
| 정소영 | 인프라 CI/CD 구축<br/> 회원 & 전체 페이지 프런트 디자인<br/> 이벤트 담당 (경제 용어 퀴즈, 행운의 금융 룰렛, 돈 먹기 게임, 물가 맞추기 게임)<br/>DailyScrum 및 팀 회의 기록 정리 <br/> Porting Manual 및 README 작성 |  
