...
PWA란? 웹 기술을 활용해 네이티브 앱처럼 작동하는 웹 애플리케이션입니다. PWA 기술을 활용하면 PC에서 뿐 아니라 모바일에서도 서비스에 접근할 수 있게 됩니다. 최근에는 PC보다 모바일을 사용하는 경우가 늘어나고 있는데, 이러한 시대의 변화에 맞춰 자녀가 더욱 편리하게 에그머니 서비스를 이용할 수 있도록 하기 위해 PWA 기술을 도입하였습니다.
...
결론적으로 에그머니 서비스가 사용자에게 최상의 금융 교육 경험을 제공할 수 있도록 돕기 위해 Blue/Green 배포라는 기술을 선택하게 되었습니다.
...
| 이름 | 역할 |   
| --- | --- |
| 나경준 | [팀장] 소셜 로그인, 회원 관리, 1원 인증, QR코드, S3, 회원 관련 프런트 디자인, 발표 | 
| 곽재은 | [프런트 리더] 주식 매수 및 매도 기능, 알림, 공통 컴포넌트, PWA, 팀 내 컨벤션 관리 | 
| 김신범 | 주식 매수 및 매도 기능, 알림, Spring Boot, Chat GPT를 활용한 뉴스 요약 기능 | 
| 정지영 | 예적금, 대출 기능, Spring Boot, DB, UCC 제작, ERD 회의 기록 |
| 황우성 | 예적금, 대출 기능, Spring Boot, DB, 하둡, 스파크, SQOOP를 활용한 예적금 추천 알고리즘 제작, 금융 퀴즈 백로직, 인프라 서포트 및 멘토링 |  
| 정소영 | 인프라 CI/CD 구축, 회원 & 전체 페이지 프런트 디자인, 이벤트 (퀴즈,게임,룰렛), DailyScrum 정리 |  
