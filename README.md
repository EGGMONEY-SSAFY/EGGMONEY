...
| 곽재은 | [프런트 리더] <br/>주식 분석 및 종목 확인 기능 (차트 포함) <br/> 주식 매수 및 매도 기능 (거래내역 및 주문현황) <br/>알림 기능 (예적금 가입, 대출 심사, 출금 요청 등)<br/> 공통 컴포넌트, PWA, 팀 내 컨벤션 관리  <br/> UCC 서포트| 
...
| 황우성 | 입금 계좌 관리 기능  <br/>예금, 적금 가입 및 해지 <br/>예적금 상품 조회 기능  <br/> 대출 신청 및 승인 기능<br/>원리금균등상환, 만기일시상환 기능 구현<br/> 자산 분석 및 자산추이 그래프 구현 <br/> 하둡, 스파크, SQOOP를 활용한 예적금 추천 알고리즘 제작<br/>경제 용어 퀴즈 백로직 및 DB 구현<br/>인프라 서포트 |  
