  ...
  <div v-if="avgLoanData" class="grid grid-cols-1 grid-flow-row px-4 mt-2">
    <div class="bg-yellow-50 rounded-xl py-3">
      <div class="flex my-auto ps-3">
        ...
        <h1 class="ps-2 text-sm">이용자들의 
          <span class="underline underline-offset-2">평균 이율</span>은 아래와 같아요!</h1>
      </div>
      <div v-for="data in avgLoanData" :key="data.avg_loan_amount"
      class=" mt-3 rounded-lg grid px-3">
        <div class="flex justify-between px-2 pe-4">
          <h1 class="text-white text-sm bg-blue-500 rounded-xl p-1 px-2" v-if="data.loan_type === 'LUMPSUM'">원리금균등상환</h1>
          <h1 class="text-white text-sm bg-green-700 rounded-xl p-1 px-2" v-else>만기일시상환</h1>
          <h1 class="text-black text-base font-bold underline underline-offset-2">{{ data.avg_loan_rate.toFixed(2) }} %</h1>
        </div>
      </div>
    </div>
  </div>
