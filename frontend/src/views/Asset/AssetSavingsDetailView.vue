    ...
    <div class="bg-white rounded-lg shadow justify-between px-4 py-3 gap-3" v-if="finStore.savings">
      <div class="grid gap-3">
        ...
        <div class="grid gap-2 pe-5">
          ...
          <div class="grid grid-flow-col grid-cols-2">
            <h1>적금 이율</h1>
            <h1 class="text-end" v-if="userSavings.savingsRate">{{ userSavings.savingsRate.toFixed(2) }}</h1>
          </div>
          <div class="grid grid-flow-col grid-cols-2">
            <h1>적금 기간</h1>
  ...
  <div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center z-50 bg-gray-900 bg-opacity-50">
    <div class="bg-white p-6 rounded-lg shadow-lg w-80">
      ...
      <div class="mb-6 text-sm grid gap-1">
        <h1>해지 시에는 <span class="text-lg font-bold text-red-500 underline underline-offset-2">{{ userSavings?.balance?.toLocaleString() }}원</span> 을</h1>
        <h1>받을 수 있습니다.</h1>
      </div>
      <div class="flex justify-end">
        <button class="bg-gray-500 text-white px-4 py-2 rounded mr-2" @click="closeModal">취소</button>
        <button class="bg-red-500 text-white px-4 py-2 rounded" @click="() => deleteSavings(Number(userSavings?.savingsId))">확인</button>
      </div>
    </div>
  </div>
</div>
