<template>
    <div class="p-5 bg-gray-100 rounded-lg shadow-md">
        ...
        <div v-else-if="bankselectstep === 1" class="step">
            <p class="text-main-color text-lg font-bold mb-4" v-if="selectbank !== null">{{ bankname[selectbank]}}계좌번호를 입력해주세요</p>
            <input 
                type="text" 
        ...
        <div v-else-if="bankselectstep === 2" class="step">
            <p class="text-main-color text-lg font-bold mb-4" v-if="selectbank !== null">{{  bankname[selectbank] }} 입금내역을 확인하시고, 에그머니 뒤 4자리 숫자를 알려주세요.</p>
            <img :src="bankitems[selectbank]" alt="bank logo" class="w-12 h-12 mx-auto mb-2" v-if="selectbank !== null">
...
const sendWonAuth = async () => {
    if (selectaccount.value) {
        try {
            const response = await axios.post('http://localhost:8080/api/v1/auth/won/send', {
